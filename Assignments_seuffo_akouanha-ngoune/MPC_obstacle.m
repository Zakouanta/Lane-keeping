...
% Now, we can proceed with your MPC controller design and simulation.

% Define the cost parameters (adjust the param for the quality of the path
% planner)
Q = diag([1e3,1,2e2,2e3]);
...
e1_initial = 0.5;       % Initial lateral deviation
...
for ii = 1:mpciterations
    ...
    X1 = x_right - xmeasure(1,end)*sin(heading);   % Update global X position
    Y1 = y_right + xmeasure(1,end)*cos(heading);   % Update global Y position
    ...
    kx = generate_obstacle_constraints(obstacles, X1, Y1, min_distance, kx);
    ...
    f2 = figure(1);
    stairs(t(end) + delta*(0:1:length(u_OL)-1), u_OL), grid on, hold on,
    plot(t(end), u(end), 'bo')
    ...
    % Plot X1 and Y1 in blue, and x_des_mpc and y_des_mpc in red
    f3 = figure(2);
    plot(X1, Y1, 'b', x_left, y_left, 'r--',x_right, y_right,'r--', ...
         x_center,y_center,'g', obstacle_1x,obstacle_1y,'bo', ...
         obstacle_2x,obstacle_2y,'bo','MarkerSize',20,'MarkerFaceColor','b');
    xlabel('X position (m)');
    ylabel('Y position (m)');
    title('Vehicle Path');
    drawnow;
