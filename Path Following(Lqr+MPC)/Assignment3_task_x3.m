...
sysc = ss(Ac, Bc, C, D);
T = 5; % Prediction time
delta = 0.09; % Sampling time for discretization
% Prediction horizon
N = fix(T/delta);
...
K_update = K_update_vel_interval(velocity_interval_mpc, boolean_param); 

%desired path with obstacles: Left, right lanes' centerlines and road
%centerline
[x_center, y_center, ~, ~, ~,obstacle_1x,obstacle_1y,obstacle_2x,obstacle_2y] = reference_path_mpc(Vx, R_mpc, L_straight, N_points);
[x_right, y_right, psidot_des_mpc, psi_des_mpc, s_ref] = reference_path_mpc_right(Vx, R_mpc, L_straight, N_points,obstacle_1x,obstacle_1y,obstacle_2x,obstacle_2y);
[x_left, y_left, ~,~, ~] = reference_path_mpc_left(Vx, R_mpc, L_straight, N_points);

% MPC Path Planning
[xref,~,~,global_X,global_Y] = SolveMPC_obstacles(Ad,Bd,n,m, ...
                                                x0,N,x_right, ...
                                                y_right,psidot_des_mpc,...
                                                psi_des_mpc,s_ref,Vx, ...
                                                obstacle_1x,obstacle_1y,...
                                                obstacle_2x,obstacle_2y); % adjust this function for the MPC solver
...
for k = 1:length(t)
    ...
    if (V_update * 3.6) >= 50 && (V_update * 3.6) < 55 - e_acc
        index = 1;
    elseif (V_update * 3.6) >= 55 && (V_update * 3.6) < 60 - e_acc
        index = 2;
    elseif (V_update * 3.6) >= 60 && (V_update * 3.6) < 65 - e_acc
        index = 3;
    elseif (V_update * 3.6) >= 65 && (V_update * 3.6) < 70 - e_acc
        index = 4;
    elseif (V_update * 3.6) >= 70 && (V_update * 3.6) < 75 - e_acc
        index = 5;
    elseif (V_update * 3.6) >= 75 && (V_update * 3.6) < 80 - e_acc
        index = 6;
    elseif (V_update * 3.6) >= 80 && (V_update * 3.6) < 85 - e_acc
        index = 7;
    elseif (V_update * 3.6) >= 85 && (V_update * 3.6) < 90 - e_acc
        index = 8;
    else
        index = 9;
    end
    ...
    [~, ~, psidot_des_mpc, psi_des_mpc, s_ref] = reference_path_mpc_right(V_update, R_mpc, L_straight, N_points,obstacle_1x,obstacle_1y,obstacle_2x,obstacle_2y);

    % Update s_current based on Vx and tmeasure
    % For a more accurate simulation, this might involve integrating over 
    % the actual path traveled, considering dynamics.
...
plot(global_X, global_Y, 'r--','LineWidth', 1.5);
hold on
plot(x_right,y_right, 'm--','LineWidth', 1.5);
hold on
plot(x_left,y_left, 'm--','LineWidth', 1.5);
hold on
plot(x_center,y_center,'k--',obstacle_1x,obstacle_1y,'bo',obstacle_2x,obstacle_2y,'bo','MarkerSize',20,'MarkerFaceColor','b','LineWidth', 1.5)

title('Vehicle Position');
xlabel('X Position (m)');
ylabel('Y Position (m)');
legend('Actual Position', 'MPC Position','Right Centerline', 'Left Centerline', 'Road centerline');
grid on;
