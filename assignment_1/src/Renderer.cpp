...
namespace lidar_obstacle_detection
{
  ...
  // Recall this function in the cluster_extraction to add the text on each cluster
  void Renderer::addText(float centroid_x, float centroid_y, float centroid_z, const std::string &id)
  {
    // Change the color to put the eventual text in white
    viewer_->addText3D(id, pcl::PointXYZ(centroid_x, centroid_y, 0), 0.5, 255, 255, 255, "", 0);
  }
