...
void ProcessAndRenderPointCloud(Renderer &renderer, pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud)
{
    ...
    **/
    ...
    for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin(); it != cluster_indices.end(); ++it)
    {
        ...
        // Use the function addText defined in renderer.cpp
        renderer.addText(centroid[0], centroid[1], centroid[2], std::to_string(distance) + " m");

        // If loop with condition the distance with respect to the ego_vehicle
        if (distance <= 5.0)
        {
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    for (const auto &entry : fs::directory_iterator("/Users/seuffoakouanhangoume/Documents/EEIV/Autonomous_Driving/dataset_2/"))
    {
