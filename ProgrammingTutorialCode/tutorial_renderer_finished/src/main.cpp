...
int main(int argc, char* argv[])
{
    ...
    while (not renderer.WasViewerStopped())
    {
        ...
        // Here it is defined the cloud to be rendered and other stuff (like color etc...)
        renderer.RenderPointCloud(input_cloud,"originalCloud",c);
        
        // Define a bounding box
        Box b{0, 0, 0,
              4, 4, 4};

        // Add to the render queue the bounding box
        renderer.RenderBox(b, 11);
        // Add to the render queue a 2D circle
        renderer.addCircle(0,0,0);
        // Add to the render queue a text
        renderer.addText(0,0,count);
        count++;
